package com.mashibing.guessingmethod_externalinformationsimplification;

/**
 * 题目三：
 * 如果一个字符相邻的位置没有相同字符，那么这个位置的字符出现不能被消掉
 * 比如:"ab"，其中a和b都不能被消掉
 * 如果一个字符相邻的位置有相同字符，就可以一起消掉
 * 比如:"abbbc"，中间一串的b是可以被消掉的，消除之后剩下"ac"
 * 某些字符如果消掉了，剩下的字符认为重新靠在一起
 * 给定一个字符串，你可以决定每一步消除的顺序，目标是请尽可能多的消掉字符，返回最少的剩余字符数量
 * 比如："aacca", 如果先消掉最左侧的"aa"，那么将剩下"cca"，然后把"cc"消掉，剩下的"a"将无法再消除，返回1
 * 但是如果先消掉中间的"cc"，那么将剩下"aaa"，最后都消掉就一个字符也不剩了，返回0，这才是最优解。
 * 再比如："baaccabb"，
 * 如果先消除最左侧的两个a，剩下"bccabb"，
 * 如果再消除最左侧的两个c，剩下"babb"，
 * 最后消除最右侧的两个b，剩下"ba"无法再消除，返回2
 * 而最优策略是：
 * 如果先消除中间的两个c，剩下"baaabb"，
 * 如果再消除中间的三个a，剩下"bbb"，
 * 最后消除三个b，不留下任何字符，返回0，这才是最优解
 *
 * 思路分析：
 * 1.创建f(L,R,K)L和R都是int类型，K是boolean类型
 * L和R表示在arr[]中的L ~ R的范围上，K表示前面有没有与arr[L]相同的数值跟着
 *
 * @author xcy
 * @date 2022/6/17 - 17:04
 */
public class Code03_DeleteAdjacentSameCharacter {
	public static void main(String[] args) {

	}

	/**
	 * 暴力解
	 * @param s
	 * @return
	 */
	public static int restMin1(String s) {
		if (s == null) {
			return 0;
		}
		if (s.length() < 2) {
			return s.length();
		}
		int minLen = s.length();
		for (int L = 0; L < s.length(); L++) {
			for (int R = L + 1; R < s.length(); R++) {
				if (canDelete(s.substring(L, R + 1))) {
					minLen = Math.min(minLen, restMin1(s.substring(0, L) + s.substring(R + 1, s.length())));
				}
			}
		}
		return minLen;
	}

	public static boolean canDelete(String s) {
		char[] str = s.toCharArray();
		for (int i = 1; i < str.length; i++) {
			if (str[i - 1] != str[i]) {
				return false;
			}
		}
		return true;
	}

	/**
	 * 优良尝试的暴力递归版本
	 * @param string
	 * @return
	 */
	public static int restMin(String string) {
		if (string == null ||string.length() == 0) {
			return 0;
		}
		if (string.length() < 2) {
			return string.length();
		}
		char[] str = string.toCharArray();
		return coreLogic(str, 0, str.length - 1, false);
	}

	/**
	 *
	 * @param str
	 * @param L
	 * @param R
	 * @param has 表示L ~ R范围之前，是否有和str[L]相同的值跟着，true表示有，false表示没有
	 * @return
	 */
	public static int coreLogic(char[] str, int L, int R, boolean has) {
		if (L > R) {
			return 0;
		}
		if (L == R) {
			//1.如果K == true，表示L ~ R范围的前面有与str[L]相同的值跟着，那么就可以消除，消除之后的长度为0
			//2.如果K == false，表示L ~ R范围的前面没有与str[L]相同的值跟着，
			//那么就只有L位置上的字符，无法消除，长度为0
			return has ? 0: 1;
		}
		//last表示最后一个与str[L]相同的值的位置
		//举例：
		//str = {'a','a',  'a',  'b','c'}
		//index = L       last        R
		int last = L;
		//K表示如果L ~ R范围之前有与str[L]相同的值跟着，前面跟着的字符长度是1，如果没有跟着，前面跟着的字符长度是0
		int K = has ? 1 : 0;
		while (last <= R && str[last] == str[L]) {
			K++;
			last++;
		}
		//如果前面跟着的字符长度K == 1，那么就无法消除掉，剩余的字符长度为1
		//如果前面跟着的字符长度K > 1，那么就可以消除掉，消除掉之后的剩余字符长度为0
		//coreLogic(str, last, R, false)表示在last ~ R范围上继续递归下去，并且前面已经没有与str[L]相同的值跟着
		int situation1 = (K > 1 ? 0 : 1) + coreLogic(str, last, R, false);
		int situation2 = Integer.MAX_VALUE;
		for (int split = last; split <= R; split++) {
			//确定首个字符是与str[L]相同的值的索引split
			//str[split] == str[L]表示split位置的数值和L位置的数值相等
			//str[split] != str[split - 1]表示split位置的数值和split - 1位置的数值不相等
			if (str[split] == str[L] && str[split] != str[split - 1]) {
				//表示在last ~ split - 1的位置上可以消除掉所有的字符
				//举例：
				//L ... last(split)  ... R
				//last来到最后一个与str[L]相同的值的位置
				//如果last ~ split之间消除之后的字符长度为0，并且前面没有与str[L]相同的值跟着
				//证明可以直接消除掉所有的字符
				if (coreLogic(str, last, split - 1, false) == 0) {
					//才能跟着后面一起消除掉
					situation2 = Math.max(situation2, coreLogic(str, split, R, K != 0));
				}
			}
		}
		//所有决策中，选择消除所有字符之后长度最小的
		return Math.min(situation1, situation2);
	}

	/**
	 * 优良尝试的动态规划版本
	 * @param s
	 * @return
	 */
	public static int restMin3(String s) {
		if (s == null) {
			return 0;
		}
		if (s.length() < 2) {
			return s.length();
		}
		char[] str = s.toCharArray();
		int N = str.length;
		int[][][] dp = new int[N][N][2];
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				for (int k = 0; k < 2; k++) {
					dp[i][j][k] = -1;
				}
			}
		}
		return dpProcess(str, 0, N - 1, false, dp);
	}

	public static int dpProcess(char[] str, int L, int R, boolean has, int[][][] dp) {
		if (L > R) {
			return 0;
		}
		int K = has ? 1 : 0;
		if (dp[L][R][K] != -1) {
			return dp[L][R][K];
		}
		int ans = 0;
		if (L == R) {
			ans = (K == 0 ? 1 : 0);
		} else {
			int index = L;
			int all = K;
			while (index <= R && str[index] == str[L]) {
				all++;
				index++;
			}
			int way1 = (all > 1 ? 0 : 1) + dpProcess(str, index, R, false, dp);
			int way2 = Integer.MAX_VALUE;
			for (int split = index; split <= R; split++) {
				if (str[split] == str[L] && str[split] != str[split - 1]) {
					if (dpProcess(str, index, split - 1, false, dp) == 0) {
						way2 = Math.min(way2, dpProcess(str, split, R, all > 0, dp));
					}
				}
			}
			ans = Math.min(way1, way2);
		}
		dp[L][R][K] = ans;
		return ans;
	}
}
