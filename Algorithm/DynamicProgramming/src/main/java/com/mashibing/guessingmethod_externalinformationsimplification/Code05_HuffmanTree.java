...
public class Code05_HuffmanTree {
	public static void main(String[] args) {
		...
		for (int i = 0; i < testTime; i++) {
			...
			String test = DynamicProgrammingUtils.randomNumberString(N, range);
	 ...
	 * 根据文章str, 生成词频统计表
	 * <p>
	 * 如果词频统计表中只有一种字符，那么将该字符编码为0就行了
	 *
	 * @param str 原始文章
	 * @return 返回str的每个字符的词频统计表
	 */
	public static HashMap<Character, Integer> countMap(String str) {
		HashMap<Character, Integer> map = new HashMap<>();
		char[] s = str.toCharArray();
		//对字符串生成的字符数组的每一个字符进行词频统计
		for (char aChar : s) {
	...
	/**
	 * 根据由文章生成的词频统计表countMap，生成哈夫曼编码表
	 * key : 字符
	 * value: 该字符编码后的二进制形式
	...
	public static HashMap<Character, String> huffmanForm(HashMap<Character, Integer> countMap) {
		...
		while (heap.size() != 1) {
			Node a = heap.poll();
			Node b = heap.poll();
			Node h = new Node(a.value + b.value);
			h.left = a;
			h.right = b;
			heap.add(h);
		}
	...
	/**
	 * 原始字符串str，根据哈夫曼编码表，转译成哈夫曼编码返回  --> 加密
	 *
	 * @param str
	 * @param huffmanForm
	 * @return
	...
	/**
	 * 原始字符串的哈夫曼编码huffmanEncode，根据哈夫曼编码表，还原成原始字符串  --> 解码
	 *
	 * @param huffmanEncode
	 * @param huffmanForm
	 * @return
	...
	/**
	 * 根据哈夫曼编码表，还原成原始字符串  --> 解码的时候，需要使用到前缀树
	 *
	 * @param huffmanForm
	 * @return
	 */
