 ...
 * - /       /  \
 * -6       3    8
 * (1).将6节点的值修改 为3，将3节点的值修改为6
 * 事实上，在工程上写这种结构的话，需要错误的节点的上下左右的环境给隔断掉，错误的节点来到另一个错误节点的环境，上下指针连好
 * (2).也可以从结构上调整对
 * 如果只是过LeetCode，可以交换两个错误节点的值
 * 但是如果真正的将题目做对，将结构连接对，是非常难的，一共14种情况
 * (3)错误节点的环境
 * -      P1           P2
 * -    /            /
 * -   e1          e2
 ...
 * 1)e1，e2有没有父节点
 * 2)e1，e2有没有挨着
 * 3)e1，e2如果有父节点的话，是左子节点还是右子节点
 * 4)函数需要返回值，返回两个错误节点交换之后新的头节点
 *
 * <p>
