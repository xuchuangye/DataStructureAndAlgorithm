 ...
 * @date 2022/4/30 - 9:45
 */
public class MaxSearchBinaryTreeHeadNode {
	...
	public static class Info {
		...
		public Info(Node root,int maxSearchBinarySubTreeSize, int totalSize, int max, int min) {
			this.root = root;
	 ...
	 * 判断左子树中的最大值是否小于当前节点
	 * 判断右子树中的最小值是否大于当前节点
	 * 如果上述情况都满足，那么整棵树就是搜索二叉树，该树的总节点个数的头节点就是最终结果
	 * @param root 二叉树的根节点
	 * @return 返回该树中满足搜索二叉树的子树的最大的节点个数的头节点
	 */
	public static Node allIsSearchBinarySubTreeMaxNodes(Node root) {
		if (root == null) {
			return null;
		}
		Info info = process(root);
		//如果该树不是搜索二叉树，返回该树中满足搜索二叉树的子树的最大节点个数的头节点
		return info.root;
	}
	...
	public static Info process(Node node) {
		...
		Node leftRoot = new Node(node.value);
		Node rightRoot = new Node(node.value);
		//该树的左子树的最大值和右子树的最小值
		int max = node.value;
		int min = node.value;
		//该树的总节点个数，默认只有根节点自己
		...
		root = leftSearchBinarySubTreeSize > rightSearchBinarySubTreeSize ? leftRoot : (rightSearchBinarySubTreeSize > maxSearchBinarySubTreeSize ? rightRoot : root);
		int masSize = leftSearchBinarySubTreeSize >rightSearchBinarySubTreeSize ? leftSearchBinarySubTreeSize : (Math.max(rightSearchBinarySubTreeSize, maxSearchBinarySubTreeSize));
		return new Info(root, masSize, totalSize, max, min);
	}


	public static Node largestBSTSubtree(Node head) {
		if (head == null) {
			return null;
		}
		return process2(head).root;
	}
	...
	public static Info process2(Node node) {
		...
		int size = p1 > p2 ? p1 : Math.max(p2, p3);
		root = p1 > p2 ? leftRoot : p2 > p3 ? rightRoot : node;
		return new Info(root, size, allSize, max, min);
	}
}
