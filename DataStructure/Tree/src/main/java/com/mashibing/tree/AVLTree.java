 ...
 * 平衡二叉树
 *
 * 因为二叉排序树可能会出现极端情况，造成根节点的左子树或者右子树某一侧的深度过长，就类似于单链表
 * 但是比单链表的查询效率更低，即使是某一侧的节点为空仍然需要进行判断
 * 所以此时就需要转换为平衡二叉树，让左子树或者右子树的深度绝对差值小于1
 * @author xcy
...
public class AVLTree {
	 ...
	 */
	public int deleteRightTreeMin(AVLNode node) {
		AVLNode target = node;
	 ...
	 */
	public int deleteLeftTreeMax(AVLNode node) {
		AVLNode target = node;
